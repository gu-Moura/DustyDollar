...
from models.entities import Account, OperationDTO, AccountStatusDTO
from services.db_service import SQLAlchemyDBService
from app_middleware import check_if_account_is_active
from exceptions import InvalidPasswordException, DatabaseWritingException

from db_models import Conta, Transacao, Pessoa

sqla = SQLAlchemyDBService()
...
def create_token():
    ...
    try:
        account = sqla.get_account_with_password(account_id, password)
...
def acc_creation():
    ...
    try:
        ...
        sqla.create_new_account(new_account, str(data.get('password')))
...
@jwt_required()
@check_if_account_is_active(db_interface=sqla)
def acc_deposit():
    data = request.get_json()
    data['operation_type'] = 'Deposit'
    deposit_data = OperationDTO.from_dict(data)
    try:
        sqla.deposit_into_account(deposit_data.account_id, deposit_data.amount)
        sqla.make_transaction(deposit_data.account_id, float(deposit_data.amount))
    except Exception:
        return jsonify({
            'status': 'error',
...
def acc_withdraw():
    ...
    try:
        if sqla.reached_withdrawal_limit(withdrawal_data.account_id, withdrawal_data.amount):
            return jsonify({
                'status': 'error',
        ...
        sqla.withdraw_from_account(withdrawal_data.account_id, withdrawal_data.amount)
        sqla.make_transaction(withdrawal_data.account_id, -float(withdrawal_data.amount))
    except Exception:
        return jsonify({
            'status': 'error',
...
def acc_block():
    ...
    try:
        sqla.change_account_active_status(data.account_id, data.account_active)
    except Exception:
        return jsonify({
            'status': 'error',
...
def acc_balance():
    ...
    balance = sqla.get_balance(account_id)
    if balance is None:
        return jsonify({
            'status': 'error',
...
def acc_statement():
    ...
    statement = [transaction.to_dict() for transaction in sqla.get_extract_from_account(account_id)]
